...
impl serde::Serialize for Wrapper<Config> {
        ...
        S: Serializer,
    {
        let adapter = TaggedSerializer {
            ...
            fks: self.me.get_foreign_keys(),
            delegate: serializer,
        };

        erased_serde::serialize(&self.me, adapter)
    }
}
    ...
    S: Serializer,
{
    ...
    fn serialize_struct(
        ...
        let mut state = self
            .delegate
            .serialize_map(Some(len + 1 + self.fks.len()))?;
        state.serialize_entry(self.tag, self.variant)?;
        for (fk_name, fk_value) in self.fks.iter() {
            state.serialize_entry(fk_name, fk_value);
        }
